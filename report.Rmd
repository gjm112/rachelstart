...
rstart %>% filter(is.na(hours_budgeted_rn)) %>%  select(2:4,clinic,hours_budgeted_rn)
...
```{r}
ggplot(aes(x = clinic, y = p_a1c) ,data = rstart) + geom_boxplot() + geom_smooth() + geom_hline(yintercept = 1, color = "red")
```

```{r}
ggplot(aes(x = clinic, y = p_bmi) ,data = rstart) + geom_boxplot() + geom_smooth() + geom_hline(yintercept = 1, color = "red")
```

```{r}
ggplot(aes(x = clinic, y = p_bp) ,data = rstart) + geom_boxplot() + geom_smooth() + geom_hline(yintercept = 1, color = "red")
```
...
rstart %>% select(clinic,p_bmi, p_bp, org_name, month) %>% filter(p_bmi > 1 | p_bp > 1) %>% write.csv("./high_bmi_or_bp_rates.csv", row.names = FALSE)

rstart %>% select(clinic,p_bmi, p_bp, org_name, month) %>% filter(p_bmi > 1 | p_bp > 1)
...
mod_bmi_assessed_all <- glm(
  cbind(n_bmi, n_total_visits_served_over18) ~  p_hours_budgeted_rn + p_hours_budgeted_ma + p_race_black + p_race_asian + p_race_other + p_patients_served_41_65 + p_patients_served_65plus + p_uninsured + p_public_insurance + p_female + p_ethnicity_hispanic,
  data = rstart,
  family = "binomial"
)
...
#Probability of uncontrolled
mod_bmi_controlled_all <- glm(cbind(n_bmi_over30, n_bmi) ~ p_hours_budgeted_rn + p_hours_budgeted_ma + p_race_black + p_race_asian + p_race_other + p_patients_served_41_65 + p_patients_served_65plus + p_uninsured + p_public_insurance + p_female + p_ethnicity_hispanic, data = rstart, family = "binomial")
